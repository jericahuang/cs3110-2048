...
(* [fix_row b row size] will remove all of the blanks of a [row]
 * to make merging easier when a move is made on the board *)
...
(* [is_empty_row b row size] returns whether or not
 * row [r] is empty in board [b] of length/width [size] *)
...
(* [process_row b s row col size] will look
 * through a [row] and look for a valid merge
...
(* [move_left b s row col size] works to complete all
 * actions of a move (shifting  and merging tiles and incrementing
 * the score accordingly) in the leftward direction as long as the
 * row is not empty *)
